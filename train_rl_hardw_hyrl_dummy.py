...
class Workspace:
    ...
    def train(self):
        ...
        # --------------------- Waypoint Prediction check ---------------------- ##
        time.sleep(6)

        waypoint = torch.tensor([-0.053, -0.290, 0.009])
        while not self.reached_waypoint:
            action = self.servoing(obs, waypoint)
            inp_action = filter_action(action)
            obs, reward, terminal, success, image_obs = self.train_env.step(inp_action)
            print("- - - - - - Waypoint1 Done - - - - -")
            obs = filter_obs(obs)
        time.sleep(1)

        init = np.array([0.0, 0.0, 0.0, 0.0])
        action = np.array([0.0, 0.0, -0.004, 0.0])
        for i in np.linspace(init, action, 10):
            i = torch.tensor(i)
            inp_action = filter_action(i)
            obs, reward, terminal, success, image_obs = self.train_env.step(inp_action)
            obs = filter_obs(obs)
        print("- - - - - - goinf down Done - - - - -")
        time.sleep(1)

        init = np.array([0.0, 0.0, 0.0, 0.0])
        action = np.array([0.0, 0.0, 0.0, 0.4])
        for i in np.linspace(init, action, 3):
            i = torch.tensor(i)
            inp_action = filter_action(i)
            obs, reward, terminal, success, image_obs = self.train_env.step(inp_action)
            obs = filter_obs(obs)
        ...
        init2 = np.array([0.0, 0.0, 0.0, 0.4])   
        action = np.array([0.0, 0.0, 0.025, 0.4])
        for i in np.linspace(init2, action, 10):
            i = torch.tensor(i)
            inp_action = filter_action(i)
            obs, reward, terminal, success, image_obs = self.train_env.step(inp_action)
            obs = filter_obs(obs)
        ...
        while not self.reached_waypoint1:
        ...
        while not self.reached_waypoint2:
        ...
        action = np.array([0.0, 0.0, -0.01, 0.4])
        for i in np.linspace(init2, action, 10):
            i = torch.tensor(i)
            inp_action = filter_action(i)
            obs, reward, terminal, success, image_obs = self.train_env.step(inp_action)
...
if __name__ == "__main__":
    ...
    main()
        ...
        # # --------------------- Waypoint Prediction check ---------------------- ##
        # action = torch.tensor([0.0, 0.0, 0.00, 0.4])
        # inp_action = filter_action(action)
        # obs, reward, terminal, success, image_obs = self.train_env.step(inp_action)
        # obs = filter_obs(obs)
        # print("- - - - - - Action1 Done - - - - -")
        ...
        # action = torch.tensor([0.0, 0.0, 0.04, 0.4])
        # inp_action = filter_action(action)
        # obs, reward, terminal, success, image_obs = self.train_env.step(inp_action)
        # obs = filter_obs(obs)
        # print("- - - - - - Action2 Done - - - - -")
        ...
        # action = torch.tensor([0.0, 0.0, -0.03, 0.4])
        # inp_action = filter_action(action)
        # obs, reward, terminal, success, image_obs = self.train_env.step(inp_action)
        # obs = filter_obs(obs)
        # print("- - - - - - Action4 Done - putting down- - - - -")
        # time.sleep(2)
