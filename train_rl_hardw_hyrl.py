...
class MainConfig(common_utils.RunConfig):
    ...
    preload_datapath: str = "release/data/real_robot_new/data_processed_new_hyrl.hdf5"
    ...
    log_per_step: int = 600
    # log
    save_dir: str = "exps/rl/train_rl_hardw_norand_hyrl_finalrun"
    use_wb: int = 0
...
class Workspace:
    def __init__(self, cfg: MainConfig, from_main=True):
        ...
        waypoint_path = f"waypoint_predictor_dual.pth"
            ...
            if mode == 'sparse':
                with torch.no_grad():
                    predicted_waypoint = self.waypoint_predictor(
                        c_image.clone().detach().to(dtype=torch.float32, device="cuda").unsqueeze(0),
