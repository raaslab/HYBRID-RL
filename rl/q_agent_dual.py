...
class QAgentConfig:
    ...
    rl_cameras: list[str] = field(default_factory=lambda: ["corner2", "eye_in_hand"])
...
class QAgent(nn.Module):
    def __init__(
        self, use_state, obs_shape, prop_shape, action_dim, rl_cameras: list[str], cfg: QAgentConfig
    ):
        super().__init__()
        ...
        if use_state:
            ...
            repr_dim = self.encoder.repr_dim * len(rl_cameras)
    ...
    def _encode(self, obs: dict[str, torch.Tensor], augment: bool) -> torch.Tensor:
        ...
        for camera in self.rl_cameras:
            data = obs[camera].float()
            if augment:
                data = self.aug(data)
            encoded_features.append(self.encoder.forward(data, flatten=False))
    ...
    def _act_ibrl(
        ...
        *,
        ...
        print(f"BC Percentage: {bc_action.size(0)/rl_bc_actions.size(0)}")
