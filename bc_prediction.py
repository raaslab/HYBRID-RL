...
class Workspace:
    def __init__(self, cfg: MainConfig, from_main=True):
        ...
        # the z_min and z_max setup
        self.reached_z_min = False
        self.reached_z_max = False
    ...
    def bc_predict(self):
        ...
        print(f"eye_in_hand shape: {obs['eye_in_hand'].shape}")
        print(obs['eye_in_hand'].is_cuda)
        ...
        for i in range(1000):
            if self.bc_policy is not None:
                ...
                if self.reached_z_max and bc_takeover == False:
                    # go_pose = np.array([-0.23, -0.35, 0.14, 3.14, 0.00, 0.002, action[3]])
                    # self.eval_env.controller._robot.move_to_eef_positions(go_pose, delta=False)
                    # bc_takeover = True
                    action[0] = -0.23 - obs['prop'][0][0]
                    action[1] = -0.35 - obs['prop'][0][1]
                    action[2] = 0.14 - obs['prop'][0][2]
                    bc_takeover = True

                if z>= z_max and self.reached_z_min == True and self.reached_z_max == False:
                    action[2] = 0.03
                    self.reached_z_max = True
            ...
            # ------ Euler - Quat ------- #
            # action = action.numpy().flatten()
            # rpy = quat_2_eul(action[3:7])
            # new_action = np.concatenate((action[:3], rpy, action[7:]))
            ...
            # #### ----- Take step -------- ##
            obs, reward, terminal, success, image_obs = self.eval_env.step(action)
